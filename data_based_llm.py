...
"""
    ...
    def generate_issue_report(self, media_name: str, reporter_name: str, issue_description: str) -> str:
        ...
        try:
            ...
            # 시스템 프롬프트: risk_report.txt(템플릿)에 '사실 우선' 최우선 원칙을 덧붙여
            # 홍보 프레이밍(평판 유지·메시지 정제)이 팩트/간결성을 누르지 못하게 한다.
            base_system = getattr(self.llm, "master_prompt", "") or ""
            override = """
...
- 간결하고 사실 중심으로 쓴다. 다음 홍보성 수식·상투어는 전면 금지: "중요한 사업", "핵심 자산/거점", "기여할/기여하고", "최선을 다하고 있습니다", "성공적 진행", "안정적으로 운영/수익 창출", "지속 가능한 공급망", "대응력을 부각/강화", "전략적 중요성".
- 사실 근거 원칙(엄수): '사실 확인'에는 [실시간 웹 검색 결과]·[과거 유사 사례]·내부 데이터에서 실제로 확인된 내용만 쓴다. 내부 운영상태·실적·수익·계약조건 등 공개되지 않은 정보를 추측으로 단정하지 말 것. 확인된 공개 사실이 없으면 지어내지 말고 "현재 공개된 구체 현황은 없음 — 담당 부서(위 유관 부서) 확인 필요"로 간결히 적는다.
- 무관 자료 인용 금지: 제공된 웹/유사사례가 이 이슈의 대상과 직접 관련이 없으면 인용하지 않는다. (예: 다른 사업·다른 자산에 관한 기사를 특정 자산의 현황 근거로 끌어붙이지 말 것.)
- 유관 의견은 해당 부서 실무자 관점의 구체적·사실적 입장으로 서술한다. 신중론(예: "대외 언급은 자제하는 것이 바람직", "구체적 사항은 정해지지 않은 단계")도 실제 보고처럼 그대로 담는다.
