...
KEYWORDS = [
    ...
    "광양 LNG",
    "우크라이나 곡물터미널",
    "포스코인터내셔널 대우",   # 대우 브랜드 가전 라이선스 (단독 '대우'는 노이즈)
    "포스코인터내셔널 팜",     # PT.PAR 등 팜 사업
    # ── 정책·복합 테마 쿼리 (T3) ──
    "대미투자특별법",
    # NOTE: "포스코인터내셔널 XXX", "포스코 XXX" 형태의 복합 쿼리는 제거함(2026-08).
    #   AND 검색이라 광역 쿼리('포스코인터내셔널' / '포스코')가 동일 기사를 이미 수집한다
    #   (3분 주기 최신 50건 → 누락 사실상 없음). 유일한 손실은 세부 해시태그 뿐이라,
    #   API 할당량 절감(하루 ~5천 회)을 위해 생략. 필요 시 여기 다시 추가.
    # ── 범용 (T4, 마지막) ──
    "포스코",
]
...
def tag_priority(keyword: str) -> int:
    """중복 기사 dedup 시 표시 태그 우선순위 (낮을수록 우선).

    포스코인터내셔널(0) > 포스코(1) > 그 외 계열사·자산·테마(2).
    같은 URL이 여러 키워드로 중복 검출될 때, 우선순위가 높은 태그가 표시되도록 한다.
    """
    k = str(keyword)
    if "포스코인터" in k or "POSCO INTERNATIONAL" in k.upper():
        return 0
    if k == "포스코" or k.startswith("포스코 "):   # 범용 '포스코' + "포스코 전기로" 등 복합쿼리
        return 1
    return 2
...
def analyze_sentiment_rule_based(title: str, summary: str) -> str:
    ...
    # 부정 키워드 (카톡 보고에서 '즉보/위기'로 다뤄진 패턴 보강)
    negative_keywords = [
        ...
        # ── 안전·재해 (최고 위기) ──
        "사망", "중대재해", "붕괴", "추락", "화재", "부상", "재해",
        # ── 수사·규제·제재 ──
        "압수수색", "영업정지", "과징금", "구속", "현장조사", "행정처분", "리콜",
        # ── 노사 ──
        "파업", "쟁의", "점거", "직고용", "노사갈등",
        # ── 재무·신용 ──
        "디폴트", "부도", "회생", "워크아웃", "강등", "기한이익상실", "EOD",
        # ── 평판·기타 ──
        "소송", "결함", "리스크", "늑장", "불성실공시", "오염"
    ]
