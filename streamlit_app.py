...
1줄: 기사 URL (그대로)
2줄: 빈 줄
3줄: 매체명 : 기사 제목 (콜론 앞뒤 공백 1칸)
4줄: 빈 줄
5줄~: 블릿 포인트 요약 (각 줄은 "- " 시작, 하이픈+공백)

[작성 규칙]
✓ 순수 텍스트만 출력 (마크다운, HTML, 이모지 절대 금지)
✓ 각 블릿은 반드시 "- " (하이픈+공백)으로 시작
✓ 블릿 개수: 4~6개 (최대 7개)
...
연합뉴스 : 포스코인터내셔널, 2023년 영업이익 1조원 돌파 전망

- 포스코인터내셔널이 2023년 영업이익 1조원을 넘어설 것으로 전망
- 3분기 누적 영업이익은 8500억원으로 전년 대비 35% 증가
- 철강 트레이딩과 에너지 사업이 실적 견인
- 4분기에도 견조한 실적이 예상되며 연간 목표 달성 가능성 높아
- 회사 측은 내년에도 성장세를 이어갈 것이라 밝혀"""
