...
# ── 사내망 TLS 검사(SSL 인터셉션) 대응 ───────────────────────────
# 사내망은 TLS를 가로채 사내 루트 CA로 재서명하는데, requests는 certifi 번들만 신뢰해
# 기사 크롤링이 전부 SSLError로 실패한다(→ 본문 0자 → 제목만 보고 요약을 지어냄).
# truststore를 쓰면 OS(Windows) 인증서 저장소를 사용해 사내 CA를 정상 신뢰한다.
# 클라우드(리눅스)에선 미설치/불필요해도 조용히 통과.
try:
...
def _extract_article_text_and_title(url: str):
    ...
    if html:
        ...
        # HTML 엔티티 복원 (&#039; &amp; &quot; 등) — 보고문 헤더에 그대로 노출되므로 필수
        if title:
...
def _format_report(llm_text: str, media: str, title: str, url: str) -> str:
    ...
    # 우수사례는 상세·[종합] 기사에서 13개까지 나옴 → 12개 캡은 마지막 불릿을 잘라먹었음
...
def make_kakao_report_from_url(url: str, fallback_media="", fallback_title="", fallback_summary=""):
    ...
    # [종합]·기획 기사는 후반부에 시장 반응·전문가 전망·상대측 입장이 나오는데
    # 6000자 컷에서 그 부분이 잘려 불릿 후반(반응·전망)이 통째로 누락됐음 → 9000자로 상향
    ...
    # ── 근거 부족 가드 ────────────────────────────────────────────
    # 본문 추출이 실패(페이월·JS 전용·차단)하면 남는 건 제목과 짧은 요약뿐인데,
    # 이 상태로 "불릿 6~13개"를 요구하면 모델이 사실 대신 그럴듯한 문장을 지어낸다.
    # (실측: 본문 0자일 때 "글로벌 입지 강화" 같은 공허한 불릿만 8개 생성됨)
    # 메신저로 그대로 나가는 문서이므로, 근거가 얇으면 창작 대신 원문 확인을 요구한다.
    MIN_EVIDENCE = 400
    if len(article_text) < MIN_EVIDENCE:
        safe_print(f"[WARN] 본문 근거 부족({len(article_text)}자) → 요약 생성 생략: {url}")
        notice = (
            "※ 기사 본문 자동 추출에 실패해 요약을 생성하지 않았습니다. "
            "원문을 확인한 뒤 직접 작성해 주세요."
        )
        blurb = (fallback_summary or "").strip()
        parts = [f"{media} : {title}"]
        if blurb:
            parts.append(f"- {blurb[:300]}")
        parts += [notice, url]
        return "\n\n".join(parts)
...
기사 1건을 받아 '도입부'와 '핵심 사실 불릿'만 작성한다.
매체명·기사제목·URL은 시스템이 자동으로 넣으므로 너는 절대 출력하지 마라.

[출력 — 정확히 이 두 가지만]
1) 도입부 (1~2문장)
   - 첫 문장: 무엇에 관한 기사인지 밝히고 "~ 관련 기사가 게재되어 보고 드립니다."로 끝맺는다.
   - 둘째 문장(선택): 사안이 복합적이면 기사 요지를 한 문장으로 압축해 "~라는 내용입니다."로 끝맺는다.
     단순·속보성 기사면 첫 문장만 쓴다.
2) 빈 줄 1개 뒤, 핵심 사실 불릿. 각 줄은 "- "로 시작하며 불릿 사이에 빈 줄 1개를 둔다.
※ 매체명/제목 헤더, URL, 기자명은 출력 금지.

[불릿 작성 — 가장 중요]
- 개수: 기사 분량에 비례해 6~13개. 짧은 속보는 5~7개, 상세·기획·[종합] 기사는 10~13개.
  기사에 담긴 사실이 많으면 불릿 수를 충분히 늘려 빠짐없이 담는다.
- 길이: 한 불릿은 대략 60~140자. 한 불릿에는 사실 1~2개만 담는다.
  서로 다른 국면(발표 내용 / 시장 반응 / 전문가 평가 / 상대측 입장)을 한 불릿에 몰아넣지 말고 각각 별도 불릿으로 쓴다.
- 완전성: 등장하는 기업·인물·기관·국가명과 날짜·금액·증감률·비중·순위 등 수치를 빠뜨리지 않는다.
  여러 항목을 "등"으로 뭉뚱그리지 말고 실제 이름을 나열한다. 직함은 정확히 표기한다.
- 순서: 기사 흐름을 그대로 따른다.
  리드(핵심 사실) → 발표·합의 세부 내용 → 경위·배경 → 시장/업계 반응·수치 → 전문가 평가·전망 → 상대측 입장·향후 일정
- 사실 간 중복 금지. 추측·해석·이모지 금지. 홍보성 수식어는 생략한다.

[문체 — 개조식 보고체]
- 모든 불릿은 명사형 종결: ~함 / ~음 / ~임 / ~됨 / ~밝힘 / ~규정 / ~지정 / ~포함 / ~경고 / ~설명 / ~답변 /
  ~마감 / ~추산 / ~전망 / ~알려짐 / ~전해짐 등 (불릿 끝에 마침표 없음)
...
→ 기사가 위 사업·그룹사·경쟁사·전현직 임원과 직접 닿을 때만 그 연결고리를 도입부에 밝힌다.
→ 관세·제재·환율·유가·타사 노사 등 업계·거시 이슈는 자사와 억지로 엮지 말고 사안 자체를 객관적으로 서술한다.
   (실제 보고 사례도 대부분 자사를 언급하지 않고 사안만 정확히 전달한다)

[좋은 예시 1 — 상세 기사(도입 2문장 + 불릿 다수, 반응·전망까지 포함)]
현대차 임금협상 관련 기사가 게재되어 보고 드립니다. 현대차 노사가 111일간의 교섭 끝에 기본급 인상과 성과급 지급 등을 골자로 한 잠정합의안을 마련하고, 파업 여파 수습 및 미래 기술 도입 등에 합의했다는 내용입니다.

- 현대자동차 노사가 기본급 10만 원 인상과 400% 이상의 성과급 지급을 골자로 한 임금협상 잠정합의안을 마련했으며, 5월 6일 상견례 이후 111일 만임

- 노사는 24일부터 울산공장에서 제17차 교섭을 진행해 잠정합의안을 도출했으며, 노조는 31일 조합원 찬반투표를 거쳐 합의안을 확정할 계획

- 잠정합의안에는 기본급 10만 원 인상(호봉 승급분 포함), 성과급 400%+1270만 원, 주식 15주, 복지포인트 50만 원, 하기 휴가비 20만 원 인상 등이 포함됨

- 이와 별도로 내년 하반기 200명, 2028년 300명 등 기술직(생산직) 500명을 신규 채용하기로 합의

- 올해 핵심 쟁점이었던 정년 연장은 법률 개정시 임금피크제 확대 없이 즉시 도입하고, 상여금 인상안은 내년 단체교섭에서 재논의하기로 함

- 현대차 노조는 7월 13일부터 부분 파업을 시작했으며, 이달 21일에는 10년 만에 하루 종일 생산을 멈추는 전면 파업을 진행

- 올해 누적 파업 시간은 60시간이며 생산라인 중단 시간은 주야 2개 조 기준 120시간에 달하고, 누적 생산차질은 5만5200대, 매출 손실은 2조3000억 원으로 추산

- 특히 피지컬 인공지능(AI), 로보틱스 등 미래 기술 도입이 기업 생존을 위한 필수 불가결한 사항이라는 데 공감대를 형성하고, 신사업·신기술 관련 진행 경과를 투명하게 공유하며 미래 산업 전환에 공동 대응하기로 함

[좋은 예시 2 — 속보성 기사(도입 1문장, 불릿 짧게)]
미국의 對캐나다 관세 유예 관련 기사가 게재되어 보고 드립니다.

- 도널드 트럼프 미국 대통령이 19일(현지시간) 발효 예정이었던 캐나다산 수입품에 대한 50% 고율 관세 부과 조치를 3일간 한시적으로 유예한다고 밝힘

- 트럼프 대통령은 18일 밤 자신의 소셜미디어를 통해 "내일 아침 발효될 예정이었던 캐나다 대상 50% 관세를 3일간 유예한다"며 문서 최종 합의를 전제로 미국과 캐나다가 '딜'에 도달했기 때문이라고 밝힘

- 트럼프 대통령은 과거 취소됐던 '키스톤 XL 유휴 석유 파이프라인' 사업을 이번 유예의 배경으로 제시했으나, 사업 재개 방식과 시점에 대해서는 구체적으로 설명하지 않음

- 이번 타결 소식은 트럼프 대통령과 마크 카니 캐나다 총리의 통화 이후 나왔으며, 주말 동안 워싱턴 DC에 머물던 캐나다 협상단이 연일 협상을 이어간 끝에 유예안을 이끌어냄

- 시장조사기관 베다 파트너스에 따르면 이번 관세 대상은 미국이 지난해 수입한 캐나다산 제품의 약 5%에 불과해 즉각적인 거시경제적 영향은 제한적일 수 있으나, 캐나다산 수출품 평균 관세율은 기존 4.68%에서 6.27%로 상승할 것으로 전망

- 전문가들은 이번 주 협상의 성패가 향후 미국·멕시코·캐나다 자유무역협정(USMCA) 갱신 전망을 가늠할 핵심 분수령이 될 것으로 전망함

위 두 예시의 도입부 형식·불릿 개수·불릿 길이·서술 순서를 그대로 따르고, 매체명·제목·URL은 출력하지 않는다."""
...
위 기사로 '도입부(1~2문장) + 핵심 불릿'만 작성하라.
불릿은 기사에 담긴 사실을 빠짐없이 담도록 6~13개로 충분히 쓰고, 한 불릿은 60~140자로 사실 1~2개만 담아라.
발표 내용·경위·시장 반응·전문가 전망·상대측 입장은 각각 별도 불릿으로 나누고, 인물·기관명과 수치는 누락하지 마라.
매체명·제목·URL은 절대 쓰지 마라 (시스템이 자동 삽입)."""
    ...
    out, _ = _openai_chat(
        ...
        # 불릿 최대 13개(각 60~140자)까지 허용하므로 출력 상한을 넉넉히 둔다.
        # (상한에 걸리면 마지막 불릿이 잘린 채 렌더돼 보고문 신뢰도가 떨어짐)
        model="gpt-4o", temperature=0.2, max_tokens=3600,
    )
    if not out and _get_gemini_key():
        out, _ = _gemini_chat(sys_prompt, user_prompt, temperature=0.25, max_tokens=3600)
